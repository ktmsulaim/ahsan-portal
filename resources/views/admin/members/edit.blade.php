                                    ...
                                    <select name="batch" id="batch" class="form-control" required>
                                        @for ($i = 1; $i <= setting('dhic_batch', 8); $i++)
                                            <option value="Batch {{ $i }}"
                                                {{ $user->batch == 'Batch ' . $i ? 'selected' : '' }}>Batch
                                                {{ $i }}</option>
                                        @endfor
                                    </select>
                                </div>
                            ...
                            <div class="col-md-12">
                                <div class="form-group">
                                    ...
                                    <select name="dhiu_dept" id="dhiu_dept" class="form-control" required>
                                        <option
                                            {{ $user->dhiu_dept == 'Department of Quran and Related Sciences' ? 'selected' : '' }}
                                            value="Department of Quran and Related Sciences">Department of Quran and
                                            Related Sciences</option>
                                        <option
                                            {{ $user->dhiu_dept == 'Department of Hadeeth and Related Sciences' ? 'selected' : '' }}
                                            value="Department of Hadeeth and Related Sciences">Department of Hadeeth and
                                            Related Sciences</option>
                                        <option
                                            {{ $user->dhiu_dept == 'Department of Fiqh and Usul ul Fiqh' ? 'selected' : '' }}
                                            value="Department of Fiqh and Usul ul Fiqh">Department of Fiqh and Usul ul
                                            Fiqh</option>
                                        <option
                                            {{ $user->dhiu_dept == 'Department of Aqeeda and Philosophy' ? 'selected' : '' }}
                                            value="Department of Aqeeda and Philosophy">Department of Aqeeda and
                                            Philosophy</option>
                                        <option
                                            {{ $user->dhiu_dept == "Department of Da'wa and Comparitive Religion" ? 'selected' : '' }}
                                            value="Department of Da'wa and Comparitive Religion">Department of Da'wa and
                                            Comparitive Religion</option>
                                        <option
                                            {{ $user->dhiu_dept == 'Department of Arabic Language and Literature' ? 'selected' : '' }}
                                            value="Department of Arabic Language and Literature">Department of Arabic
                                            Language and Literature</option>
                                        <option {{ $user->dhiu_dept == 'None of above' ? 'selected' : '' }}
                                            value="None of above">None of above</option>
                                    </select>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <select name="dhiu_batch" id="dhiu_batch" class="form-control" required>
                                    @for ($i = 13; $i <= setting('dhiu_batch', 28); $i++)
                                        <option {{ $user->dhiu_batch == 'Batch ' . $i ? 'selected' : '' }}
                                            value="Batch {{ $i }}">Batch {{ $i }}</option>
                                    @endfor
                                </select>
                            </div>
                        </div>
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <select class="form-control" name="district" id="district" required>
                                        ...
                                        <option {{ $user->district == 'ALAPPUZHA' ? 'selected' : '' }} value="ALAPPUZHA">
                                            ALAPPUZHA</option>

                                        <option {{ $user->district == 'ERNAKULAM' ? 'selected' : '' }} value="ERNAKULAM">
                                            ERNAKULAM</option>

                                        <option {{ $user->district == 'IDUKKI' ? 'selected' : '' }} value="IDUKKI">IDUKKI
                                        </option>

                                        <option {{ $user->district == 'KANNUR' ? 'selected' : '' }} value="KANNUR">KANNUR
                                        </option>

                                        <option {{ $user->district == 'KASARAGOD' ? 'selected' : '' }} value="KASARAGOD">
                                            KASARAGOD</option>

                                        <option {{ $user->district == 'KOLLAM' ? 'selected' : '' }} value="KOLLAM">KOLLAM
                                        </option>

                                        <option {{ $user->district == 'KOTTAYAM' ? 'selected' : '' }} value="KOTTAYAM">
                                            KOTTAYAM</option>

                                        <option {{ $user->district == 'KOZHIKODE' ? 'selected' : '' }} value="KOZHIKODE">
                                            KOZHIKODE</option>

                                        <option {{ $user->district == 'MALAPPURAM' ? 'selected' : '' }}
                                            value="MALAPPURAM">MALAPPURAM</option>

                                        <option {{ $user->district == 'PALAKKAD' ? 'selected' : '' }} value="PALAKKAD">
                                            PALAKKAD</option>

                                        <option {{ $user->district == 'PATHANAMTHITTA' ? 'selected' : '' }}
                                            value="PATHANAMTHITTA">PATHANAMTHITTA</option>

                                        <option {{ $user->district == 'THIRUVANANTHAPURAM' ? 'selected' : '' }}
                                            value="THIRUVANANTHAPURAM">THIRUVANANTHAPURAM</option>

                                        <option {{ $user->district == 'THRISSUR' ? 'selected' : '' }} value="THRISSUR">
                                            THRISSUR</option>

                                        <option {{ $user->district == 'WAYANAD' ? 'selected' : '' }} value="WAYANAD">
                                            WAYANAD</option>
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <div class="form-group">
                                    ...
                                    <textarea class="form-control" name="address2" id="address2">{{ old('address2', $user->address2) }}</textarea>
                                </div>
                            </div>
                        </div>
                                    ...
                                    <div>
                                        <div class="custom-control custom-checkbox-toggle custom-control-inline mr-1">
                                            <input type="checkbox" name="marital_status" id="marital_status"
                                                ...
                                                {{ $user->marital_status == 1 ? 'checked' : '' }}
                                                value={{ $user->marital_status == 1 ? true : false }}>
